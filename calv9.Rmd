...
df = dplyr::select(df, symbol, date, close) # Select the cols we need for the research
...
df$available_returns = rowSums(!is.na(df[5:15])) # compute number of available returns
df$cum_return = matrixStats::rowProds(1+as.matrix(df[5:15]), na.rm=T)
```
...
decile_seperation = function(portfolio){
  x = portfolio %>% dplyr::filter(available_returns >= 8) # Above 8 monthly returns in ranking 
  ...
  count = 1 #counter for usage in progress bar
    for (j in 1:length(reb_month)){ # Takes all months with the mentioned criteria above.
      temp_holder = x %>% dplyr::filter(date == reb_month[j]) # filter each dates so we have one specific month
...
df = decile_seperation(df) %>% as_tibble() # calls the function on tibble df, and saves as dfa  
...
#                        DOES EQUALY WEIGHTED PORTFOLIO
###################################################################
...
dfx = df %>% filter(quantile == 1) %>% dplyr::filter(date == "2010-12-31") # Checking that weights sum to one 
