...
df = df %>% dplyr::filter(date > "2005-01-01") 
...
df$cum_return_norm = df$cum_return / df$available_returns #Normalising our returns by devided on number on total number of rets
df = df %>% dplyr::filter(available_returns >= 8) # Above 8 monthly returns in ranking
      ...
      setTxtProgressBar(pb,count) # Starting the progress bar
        for (j in 1:10){ 
          ...
          if (j == 1 && i == 1) { # This is only here to remoe the one binding col!
             temp_holder = temp_holder %>% dplyr::slice(-c(nrow(temp_holder)))
             }
        }
