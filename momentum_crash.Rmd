...
```
The kf_monthly dataset is very strange!!!! Beaware of wierd numbers!!!!!
```{r}
...
```{r}
  ggplot(kf_monthly, aes(x = Date)) +
    geom_smooth(aes(y = Lo.PRIOR), color = "darkred") +
    ...
    geom_smooth(aes(y = Hi.PRIOR), color = "Yellow") +
    labs(x = 'Date',
       y = 'Cumulative Returns',
       title = 'Portfolio Cumulative Returns') +
  #theme_classic() +
  scale_y_continuous(breaks = seq(-10,10000,500)) +
...
```{r}
kf_daily = kf_daily %>% 
  mutate(Ret_lo.prior = cumprod(1+(Lo.PRIOR/100)))
