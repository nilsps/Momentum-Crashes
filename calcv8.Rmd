...
date_filter = function(portfolio) {
  ...
  for (i in 1:years) {
    year_counter = (2005 + counter) ###############HARDCODED 2000 NEEDS TO BE CHANGED ACCORDING TO YEAR! ###########
...
reg_top = roll_regres(norm_ret_mont_top ~ norm_sp500_ret, month_ret_tot, width = 6, #Testing top vs SP500, 6 month window
            do_compute = c("sigmas", "r.squareds", "1_step_forecasts"))

reg_bot = roll_regres(norm_ret_mont_bot ~ norm_sp500_ret, month_ret_tot, width = 6, #Testing top vs SP500, 6 month window
            do_compute = c("sigmas", "r.squareds", "1_step_forecasts"))
...
ggplot(reg_top_sum, aes(x = date)) + # Uses main dataset
       ...
       geom_line(data = reg_bot_sum, aes(y = beta_bot, color = "bot"), size = 0.5) +
       #geom_line(aes(y = bot, color = "bot"), size = 1) + 
       #geom_line(aes(y = mid, color = "mid"), size = 1) + 
       #geom_line(aes(y = wml, color = "wml"), size = 1) + 
       #geom_line(aes(y = cu_ff_mkt_rf, color = " cu_ff_mkt_rf"), size = 1) + 
...
ts_df = ts(log(ts_totscore), start = c(2005, 1), freq = 12)
plot(ts_df)
plot(ts_df^2)
