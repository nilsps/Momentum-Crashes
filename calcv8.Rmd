...
start_year = 2000 # Important to set this equal to top ! Will be used further down
...
colors <- c("top" = "blue", "bot" = "orange", "wml" = "green" , "sp500" = "red", "mid" = "brown")
...
month_ret_tot = cbind(month_ret_tot,sp500_dates$return)
names(month_ret_tot)[5] = "sp500_ret"
...
ts_sp500_ret = ts(sp500_dates$return, start = c(start_year, 1), freq = 12) # Creating time series
...
total_score$alpha = ts_top_ret - total_score$ff_rf - beta * (total_score$ff_mkt_rf - total_score$ff_rf) # calculating alpha. Think this might be redundant 
```
...
month_ret_tot$norm_sp500_ret = normalizefunc(month_ret_tot, 5)
...
reg_mid = roll_regres(norm_ret_mont_mid ~ norm_sp500_ret, month_ret_tot, width = 6, #Beta top vs SP500, 6 month window
            do_compute = c("sigmas", "r.squareds", "1_step_forecasts"))
...
ts_sp500_close = ts(temp_close_sp[,1], start = c(start_year, 10), freq = 12) # closing prices
ts_sp500_ret = ts(temp_sp500_ret[,1], start = c(start_year, 10), freq = 12) # returns  
ts_sp500_rvol = ts(temp_sp500_rvol[,1], start = c(start_year, 10), freq = 12) # real volatility
```
...
cumalative_ret_dynamic = function (portfolio,portfolio2, gjr_vol){ # Defining function
  reb_month =  unique(portfolio$date) # finds each re-balancing months from the portfolio
  tot_ret = c() # empty vector 
  border = 2.4 # Selecting our volitility cap 
    for (i in 1:length(reb_month)){ # for each date we have in the portfolio do this 
      if (i == 1){  
...
top_2 = top_1 %>% dplyr::filter(date > start_date ) # Date selection of portfolio is just here for testing 
bot_2 = bot_1 %>% dplyr::filter(date > start_date)
...
garchvol1 = garchvol1 %>% dplyr::filter(date > start_date)
...
```{r}
colors = c("top" = "blue", "bot" = "orange", "set_dynamic" = "green", "sp500" = "red", "USREC" = "purple")
...
#sp_test = sp_test[1:1]
x = x#[72:nrow(x),]
ts_sp_test = ts(x[3], start = c(start_year,10), frequency = 12 )

garchspec <- ugarchspec(mean.model = list(armaOrder = c(0, 0), archm = TRUE, archpow = 2), variance.model = list(model = "gjrGARCH", garchOrder = c(1, 1)), distribution.model = "sstd")
...
# ts_temp_top_ret = ts(temp_top_ret[1:100,1] /100, start = c(start_year, 10), freq = 12)
# 
# temp_norm_top_ret = month_ret_tot %>%  dplyr::select("norm_ret_mont_top","date") %>% arrange(date)
# #temp_norm_top_ret = temp_norm_top_ret[,1] %>% log()
# temp_norm_top_ret = ts(temp_norm_top_ret[,1], start = c(start_year, 10), freq = 12)
# 
# 
# 
...
#                        distribution.model = "sstd")
# 
# gjrfit = ugarchfit(garchspec, temp_norm_top_ret)
# 
# forcast_gjr_top = ugarchforecast(fitORspec = gjrfit, n.ahead = 12) 
# 
# 
# 
# 
# #predic = as.data.frame(forcast_gjr_top)
# 
...
```
```{r}
ts_sp = ts(month_ret_tot[2], start = c(start_year,10), frequency = 12 )
...
garchfit = ugarchfit(data = ts_sp, spec = garchspec)
...
vol_calc = function (data,period){
  width = period
  nulls = rep(0, width-1) %>% as_tibble
  temp = rollapply(data, width = width, FUN = "sd") %>% as.tibble()
  temp = temp * sqrt(temp)
...
x = vol_calc(month_ret_tot$norm_ret_mont_top,width)
wml_chris$top_vol_6 = x$value

x = vol_calc(month_ret_tot$norm_ret_sp500,width)
wml_chris$sp500_vol_6 = x$value

x = vol_calc(month_ret_tot$norm_ret_mont_bot,width)
wml_chris$bot_vol_6 = x$value
```
...
wml_chris$f_top_vol = forward_vol_calc(month_ret_tot$norm_ret_mont_top)
```

```{r}
 
a_constant = rep(1, nrow(month_ret_tot))

reg_ftop_sigma = lm(wml_chris$f_top_vol ~ garchfit@fit$sigma)
reg_ftop_126 = lm(wml_chris$f_top_vol ~ wml_chris$top_vol_6)
reg_ftop_cons = lm(wml_chris$f_top_vol ~ a_constant)

summary(reg_ftop_sigma)
summary(reg_ftop_126)
summary(reg_ftop_cons)
```


```{r}
#so we need to make rolling variance first

var(sp500_dates$return)

ts_sp500_v1 = sp500_dates %>% dplyr::select(return,date) 
ts_sp500_v1 = ts_sp500_v1 %>% ungroup() %>% dplyr::select(-c(symbol))

a = c(0,0,0,0,0) %>% as.matrix
rollinvar = rollapply(sp500_dates$return, width = 6, by = 1, FUN = "var") %>% as.tibble()

colnames(a)[1] = "value" 
matchroll = rbind(a,rollinvar)

y_hat0 = 1.973/100
y_int = -0.397

wml_chris = wml_chris %>% mutate(yhat = y_hat0)
wml_chris = wml_chris %>% mutate(yint = y_int)
wml_chris = wml_chris %>% mutate(roll_6_var = matchroll$value)

wml_chris$roll_6_var %>% plot(type = "l")
wml_chris$f_top_vol %>% plot(type = "l")

# Bear indicator = USREC 
wml_chris[28:ncol(wml_chris)]

errorterm = 0 # Expected value of noise

wml_chris$r_hat = wml_chris$yhat + (wml_chris$yint * wml_chris$USREC * wml_chris$roll_6_var) + errorterm
...
```{r}
  ...
  wml_chris$garch_res = garchfit@fit$residuals
  
  
  wml_chris$binary_error[wml_chris$garch_res > 0] = 0
  wml_chris$binary_error[sapply(wml_chris$binary_error, is.na)] = 1
  ...
  sigma_garch = omega + (beta*wml_chris$top ) + (alpha + gamma * wml_chris$binary_error) * wml_chris$garch_res^2 
  
  # sigma_roll = roll_regres(wml_chris$ ~ norm_sp500_ret, wml_chris, width = 1, #Beta top vs SP500, 6 month window
  #             do_compute = c("sigmas", "r.squareds", "1_step_forecasts"))
  
  
  # a = c(0,0,0,0,0) %>% as.matrix
  # colnames(a)[1] = "value" 
  # roll_6_month_var = rollapply(month_ret_tot$ret_mont_top, width = 6, by = 1, FUN = "var") %>% as.tibble()
  # matchroll = rbind(a,roll_6_month_var)
  
  
  
  sigma22 = 0.001 + sigma_garch * 0.6114 + wml_chris$top_vol_6 * 0.264
  
  sig22mean = sigma22%>% mean() 
  
  # roll_1_forward_var = rollapply(month_ret_tot$ret_mont_top, width = 6, by = 1, FUN = "var") %>% as.tibble()
  # matchroll = rbind(a,roll_6_month_var)
...
```{r}
v = month_ret_tot$ret_mont_top %>% mean()

scalar = (v / sig22mean)

weights = (1/(2*scalar)) * (wml_chris$r_hat/sigma22) *100
weights %>% max() 
weights %>% min()
...
mod_weights$mod[mod_weights$value > 1] = 1.032
...
top_score_dyn %>% plot(type ="l")

volatility(top$return)
...
```{r}
colors = c("top" = "blue", "bot" = "orange", "set_dynamic" = "green", "sp500" = "red", "USREC" = "purple", "top_dync" = "black")
...
ggplot(tot_score_final, aes(x = date)) + # Uses main dataset
       ...
       geom_line(aes(y = sp500_cumret, color = "sp500"), size = 1)+
       geom_line(aes(y = USREC/3, color = "USREC"), size = 1)+
       geom_line(aes(data = top_score_dyn, y= top_score_dyn, color = "top_dync"), size = 1)+
       labs(x = 'Date',
       y = 'Cumulative Returns',
       title = 'Portfolio Cumulative Returns') +
    scale_y_continuous(breaks = seq(0,15,1)) +
...
char_top = table_formating(ts_top_ret,ts_sp500_ret) %>% as_tibble()
char_mid = table_formating(ts_mid_ret,ts_sp500_ret)%>% as_tibble()
char_bot = table_formating(ts_bot_ret,ts_sp500_ret)%>% as_tibble()

char_tot = cbind(char_top,char_mid,char_top)
colnames(char_tot)[1:3] = c("top","mid","bot")
