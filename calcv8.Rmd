...
#auto.arima(decompose_ts_top,D=1, approximation = FALSE, lambda = lambda, parallel = TRUE, stepwise=FALSE, num.cores = 20 )
```
...
row.names(temp_sp500) = temp_sp500$date
...
ts_sp500_close = ts(temp_close_sp, start = c(2005, 10), freq = 12) # closing prices
ts_sp500_ret = ts(temp_sp500_ret, start = c(2005, 10), freq = 12) # returns  
ts_sp500_rvol = ts(temp_sp500_rvol, start = c(2005, 10), freq = 12) # real volatility
```
...
ts_sp500_close = ts_sp500_close[1:40]
...
spec_final= garch_sp500_spec
setfixed(spec_final) = as.list(coef(forcast_gjr_top))

f2005 = ugarchforecast(data = ts_sp500_ret[1:32], 
                       fitORspec = spec_final,
                       n.ahead = 12)

f2008 = ugarchforecast(data = ts_sp500_ret[36:48], 
                       fitORspec = spec_final,
                       n.ahead = 12)
plot(sigma(f2005))

sim = ugarchpath(spec = spec_final,
                 m.sim = 3,
                 n.sim = 12,
                 rseed.=321)
...
spec = garch_sp500_spec

ugarch_roll = ugarchroll(spec, garch_sp500_spec, 
                  refit.every = 6, 
                  forecast.length = 12, 
                  refit.window = "moving", 
                  solver = "hybrid", 
                  ...
                  VaR.alpha = c(0.01, 0.05, 0.1, 0.15), 
                  cluster = cl, 
                  keep.coef = TRUE
                  )
