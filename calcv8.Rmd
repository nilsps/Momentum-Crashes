...
date_filter = function(portfolio) {
  ...
  for (i in 1:years) {
    year_counter = (2000 + counter)
    x = selectByDate(portfolio, year = year_counter ) ###############HARDCODED 2000 NEEDS TO BE CHANGED ACCORDING TO YEAR!
    counter = counter + 1
    setTxtProgressBar(pb,counter) # Starting the progress bar
    for (j in 1:12){
      temp_holder1 = x[months(x$date) %in% month.name[j],]
      temp_holder1 = temp_holder1[nrow(temp_holder1),]
      temp_holder2 = rbind(temp_holder1,temp_holder2)
    }
...
ts_sp500_ret = ts(sp500_dates$return, start = c(2000, 10), freq = 12)
ts_top_ret = ts(month_ret_tot$ret_mont_top , start = c(2000, 10), freq = 12)
ts_bot_ret = ts(month_ret_tot$ret_mont_bot , start = c(2000, 10), freq = 12)

##
## Make a risk free rate ts ? is it neccecery?! 


CAPM.beta(ts_top_ret, ts_sp500_ret, Rf = 0) # THIS Works in our test!  
...
reg_top = roll_regres(ret_mont_top ~ sp500_ret, month_ret_tot, width = 6, #Testing top vs SP500, 6 month window
            do_compute = c("sigmas", "r.squareds", "1_step_forecasts"))

reg_bot = roll_regres(ret_mont_bot ~ sp500_ret, month_ret_tot, width = 6, #Testing top vs SP500, 6 month window
            do_compute = c("sigmas", "r.squareds", "1_step_forecasts"))
...
# 
# ts_reg_top_sum = ts(reg_top_sum$beta_top[,1], start = c(2000, 10), freq = 12)
# ts_reg_top_sum_test = ts(reg_top_sum$beta_top, start = c(2000, 10), freq = 12)

ts_reg_top_sum %>% plot()


reg_top_sum %>% ggplot( aes(x=date, y=beta_top))+
  geom_line()+ylab("Rolling Beta")+
  ggtitle("Rolling Beta  top vs sp500")

colors <- c("top" = "blue", "bot" = "orange", "wml" = "green" , " cu_ff_mkt_rf" = "red", " mid" = "black")

ggplot(reg_top_sum, aes(x = date)) + # Uses main dataset
       geom_line(aes(y = beta_top, color = "top"), size = 1) +
       #geom_line(aes(y = bot, color = "bot"), size = 1) + 
       #geom_line(aes(y = mid, color = "mid"), size = 1) + 
       #geom_line(aes(y = wml, color = "wml"), size = 1) + 
       #geom_line(aes(y = cu_ff_mkt_rf, color = " cu_ff_mkt_rf"), size = 1) + 
