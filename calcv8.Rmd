...
df = dplyr::filter(df, date > "1990-12-31")
...
df$norm_norm_cret = normalized = (x-min(x))/(max(x)-min(x))
```
...
decile_seperation = function(portfolio){
  ...
  x = x %>% dplyr::distinct(norm_norm_cret, .keep_all = T)
  ...
  count = 1 #counter for usage in progress bar
    for (j in 1:length(reb_month)){ # Takes all months with the mentioned criteria above.
      ...
      temp_holder1$quantile = cut(temp_holder1$norm_norm_cret, # put it into dociles, based on monthly return
          quantile((1+temp_holder1$norm_norm_cret),probs=seq(from=0,to=1,by=1/10),na.rm =T),
          #quantile((normalized_cumalative),probs=seq(from=0,to=1,by=1/10),na.rm =T),
          include.lowest=TRUE, 
          labels=FALSE) # %>% as.list() %>% as.integer()
       temp_holder2= bind_rows(temp_holder1 ,temp_holder2)  # Binds the score and the folder for a stock and date
       count = count + 1 # Adds 1 to the counter for progressbar 
       setTxtProgressBar(pb,count) # Starting the progress bar
    }  
  return(temp_holder2) # Returns from function a entire dataset with quantiles attached.
}
...
ts_df = ts(log(ts_test$return+1), start = c(1990, 10), freq = 12)
plot(ts_df)
plot(ts_df^2)
...
ts_df = ts(log(ts_totscore$top), start = c(1991, 10), freq = 12)
plot(ts_df)
plot(ts_df^2)
