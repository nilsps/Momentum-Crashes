...
month_day = LastDayInMonth(sp500$date) %>% as.Date()

month_day = month_day %>% unique()
 # This is due to extreme amounts of NAs in 1970s-1960 to get a better dataset. 

sp500 = dplyr::filter(sp500, date == month_day) #Filters out all other dates for the month except last one

transform(sp500, date = bsts::LastDayInMonth(sp500$date))
...
return_function_sp500 = function(portfolio){
  x = ((lead(portfolio$close) - portfolio$close)  / portfolio$close)
  return(x)
}

sp500$return = return_function_sp500(sp500)

sp500 %>%
  mutate(cum_ret = cumprod(1 + sp500$return)) 
...
wml_chris$forcast = ((wml_chris$forcast - 1)*100) 

wml_chris$forcast_2 = ((((wml_chris$forcast)^2 - wml_chris$forcast)^2))
