...
colors <- c("top" = "blue", "bot" = "orange", "wml" = "green" , "sp500" = "red")
...
cumalative_ret_dynamic = function (portfolio,portfolio2, gjr_vol){ # Defining function
  reb_month =  unique(portfolio$date) # finds each re-balancing months from the portfolio
  tot_ret = c() # empty vector 
  border = 0 # Selecting our volitility cap 
    for (i in 1:length(reb_month)){ # for each date we have in the portfolio do this 
      if (i == 1){  
...
dynamic = cumalative_ret_dynamic(top_2,bot_2,garchvol1) # runs the script
...
ggplot(tot_score_final, aes(x = date)) + # Uses main dataset
       geom_line(aes(y = top, color = "top"), size = 1) +
       geom_line(aes(y = bot, color = "bot"), size = 1) + 
       geom_line(aes(y = dynamic, color = "dynamic"), size = 1) + 
       geom_line(aes(y = sp500_cumret, color = "sp500"), size = 1)+
...
wml_chris$weights_forcast = (1/2*(month_ret_tot$lambda)) * (wml_chris$forcast / wml_chris$forcast_2)
...
```{r}
ts_sp = ts(month_ret_tot[2], start = c(2000,10), frequency = 12 )
...
sigma_garch = coefs[3] + (coefs[5]*wml_chris$roll_6_var) + (coefs[4] + coefs[6] * wml_chris$binary_error) * wml_chris$garch_res^2 

sigma_roll = roll_regres(norm_ret_mont_top ~ norm_sp500_ret, month_ret_tot, width = 6, #Beta top vs SP500, 6 month window
            do_compute = c("sigmas", "r.squareds", "1_step_forecasts"))


a = c(0,0,0,0,0) %>% as.matrix
colnames(a)[1] = "value" 
roll_6_month_var = rollapply(temp_top_ret$ret_mont_top, width = 6, by = 1, FUN = "var") %>% as.tibble()
matchroll = rbind(a,roll_6_month_var)



sigma22 = 0.001 + sigma_garch *0.6114 + sigma^ of last 6 month rolling windown  
