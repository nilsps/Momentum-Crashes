...
df = subset(df, select = -c(open, high, low, volume) )
...
df
return_cols
df$available_returns=rowSums(!is.na(df[,return_cols, with=FALSE])) # compute no of available returns
df



dfa<-NULL
df$cum_return<-rowProds(1+as.matrix(df[,return_cols, with=FALSE]), na.rm=T) # compute cumulative product
df
df<-df[, !return_cols, with=FALSE] # drop return_cols (not needed anymore)
return_cols<-NULL
df
```
