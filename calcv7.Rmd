...
```{r}
df$available_returns = rowSums(!is.na(df[9:19])) # compute number of available returns
df$cum_return = rowProds(1+as.matrix(df[9:19]), na.rm=T)
    ...
    for (i in 1:length(reb_month)){ # for each date we have in the portfolio do this 
      ...
      setTxtProgressBar(pb,count) # Starting the progress bar
        for (j in 1:10){ 
          ...
          tot_w = sum(a$mcap)  
          temp_holder = (a$mcap/tot_w) %>% as.data.frame()
          temp_holder2 = bind_rows(temp_holder,temp_holder2)
      }
    }
  return(temp_holder2)
...
vw  = v_weights(df)
vw = vw %>% slice(-c(nrow(vw)))
names(vw)[names(vw) == "."] <- "vw"
df = cbind(df,vw )
```
