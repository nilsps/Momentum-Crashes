...
df = read_delim("db_dat/asx200.csv",  ";", escape_double = FALSE, locale = locale(decimal_mark = ","), na = "NA", trim_ws = TRUE)
...
index_use = c("^AXJO") # Select the index to load here 
...
```{r}
index_tables = total_score

ts_sp = ts(month_ret_tot$norm_ret_index, start = c(start_year,10), frequency = 12 )
...
x = vol_calc(month_ret_tot$norm_ret_index,width)
...
index_tables$f_top_vol = forward_vol_calc(month_ret_tot$norm_ret_mont_top)
```
...
reg_ftop_sigma = lm(index_tables$f_top_vol ~ garchfit@fit$sigma)
reg_ftop_126 = lm(index_tables$f_top_vol ~ index_tables$top_vol_6)
reg_ftop_cons = lm(index_tables$f_top_vol ~ a_constant)
...
ts_index_v1 = index_dates %>% dplyr::select(return,date) 
ts_index_v1 = ts_index_v1 %>% ungroup() %>% dplyr::select(-c(symbol))

a = c(0,0,0,0,0) %>% as.matrix
rollinvar = rollapply(index_dates$return, width = 6, by = 1, FUN = "var") %>% as.tibble()
...
index_tables$garch_res = garchfit@fit$residuals
  
  
index_tables$binary_error[index_tables$garch_res > 0] = 0
index_tables$binary_error[sapply(index_tables$binary_error, is.na)] = 1
...
```{r}
colors = c("top" = "blue", "bot" = "orange", "set_dynamic" = "green", "index" = "red", "USREC" = "purple", "dynamic" = "black")
