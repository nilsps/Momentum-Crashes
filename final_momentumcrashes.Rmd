...
names(month_ret_tot)[9] = "norm_ret_sp500"
...
cumalative_ret_dynamic = function (portfolio,portfolio2, gjr_vol){ # Defining function
  reb_month =  unique(portfolio$date) # finds each re-balancing months from the portfolio
  tot_ret = c() # empty vector 
  border = 3 # Selecting our volitility cap 
    for (i in 1:length(reb_month)){ # for each date we have in the portfolio do this 
      if (i == 1){  
...
```{r}
ts_sp = ts(month_ret_tot$norm_ret_sp500, start = c(start_year,10), frequency = 12 )
...
wml_chris$r_hat = wml_chris$yhat + (wml_chris$yint * wml_chris$USREC * wml_chris$sp500_vol_6 ) + errorterm
...
end = top_score_dyn %>% length()
split = end / 2

returns = 1+top_score_dyn[1+split:end] %>% na.omit() # removes 1 na col at highest date, due to erorr calc

top_score_dyn = top_score_dyn[1:split] %>% na.omit() 
```

```{r}
library(rollRegres) # Using rolling regression 

normalizefunc = function(portfolio){ # Making the normf. funk 
  x = portfolio
  y = (x-min(x))/(max(x)-min(x))
  return(y)
}
...
reg_dyn = roll_regres(top_score_dyn$norm_score ~ month_ret_tot$norm_ret_sp500 , width = 6, #Beta top vs SP500, 6 month window
            do_compute = c("sigmas", "r.squareds", "1_step_forecasts"))
...
colnames(tot_score_final)[ncol(tot_score_final)] = "dynamic_fancy"
```

```{r}
colors = c("top" = "blue", "bot" = "orange", "set_dynamic" = "green", "sp500" = "red", "USREC" = "purple", "dynamic" = "black")
...
ggplot(tot_score_final, aes(x = date)) + # Uses main dataset
       ...
       geom_line(aes(y = sp500_cumret, color = "sp500"), size = 1)+
       geom_line(aes(y = USREC/3, color = "USREC"), size = 1)+
       geom_line(aes(y = dynamic_fancy, color = "dynamic"), size = 1)+
       labs(x = 'Date',
       y = 'Cumulative Returns',
       title = 'Monthly dynamic leverages possitive and negative direction') +
