...
# EW_top = 1/nrow(top_portf)
# EW_top_ivest = 1/nrow(top_portf)

#top_portf = top_portf %>% mutate(cr_port = 1)

holder = c()
for (i in 1:somethign){ 
reb_month_top =  unique(top$date)   
top_portf = top[1:3] %>% filter(date == as.Date(reb_month_top[1]))
}
...
#ret_port = function(top_portf){
  for (i in 1:nrow(top_portf)){
    ...
    z = df %>% filter(stock_id == y[1,1]) # all returns for the specific stock chosen
    m = x[1,2]  %m+% months(1) # returns the stock plus one month 
    n = df %>% filter(stock_id == x[1,1]) %>% filter (date == m) # locates the return for this stock
