...
```{r}
df$return = (lead(df$adjusted)- df$adjusted)  / lead(df$adjusted) # Calculates daily returns 
...
x =(return_calculations(top))
top$port_ret = x 
```

```{r}
bot = dfa %>% filter(quantile == 1) %>% arrange(date) # making the best performance portf 
#top = top[1:200,]
...
return_calculations = function(portfolio){
  ret = c(1:length(portfolio))
  reb_month_top =  unique(portfolio$date)
  v = portfolio[1:3]
  pb = txtProgressBar(min = 0, max = nrow(portfolio), initial = 0, style = 3)
  count = 1
    for (j in 1:length(reb_month_top)){
