...
```{r}
top = dfa %>% dplyr::filter(quantile == 10) %>% arrange(date)%>% na.omit() # making the best performance portf
mid = dfa %>% dplyr::filter(quantile == 5) %>% arrange(date) %>% na.omit()
bot = dfa %>% dplyr::filter(quantile == 1) %>% arrange(date) %>% na.omit()
...
####### Cumalative return calculation 

cumalative_ret_portfolio = function (portfolio){ 
reb_month =  unique(portfolio$date)
portfolio = portfolio #%>% group_by(date)
